...
const ChallengeDetailPage = () => {
    ...
    const fetchFeed = async () => {
        try {
            const res = await fetch(`${API_BASE_URL}/challenges/${id}/feed`, {
                headers: { Authorization: `Bearer ${getToken()}` }
            });
            const data = await res.json();
            if (!res.ok) throw new Error(data.error);
    ...
    const handleLeave = async () => {
        ...
        if (!confirmed) return;
        try {
            const res = await fetch(`${API_BASE_URL}/challenges/${id}/leave`, {
                method: "POST",
                headers: { Authorization: `Bearer ${getToken()}` }
            });
            const data = await res.json();
    ...
    const handleMarkDone = async () => {
        setMarkingDone(true);
        try {
            const res = await fetch(`${API_BASE_URL}/challenges/${id}/complete-today`, {
                method: "POST",
                headers: { Authorization: `Bearer ${getToken()}` }
            });
            const data = await res.json();
    ...
    const handleReact = async (postId, emoji) => {
        try {
            const res = await fetch(`${API_BASE_URL}/challenges/${id}/feed/${postId}/react`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${getToken()}`
